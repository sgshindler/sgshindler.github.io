---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* Ph.D in Chemical and Biomolecular Engineering, Cornell University, 2025
* B.S. in Chemical Engineering with Math Minor, University of Idaho, 2019

Experience
======
Fall 2025 - Technical Lead, Ultinat Inc., Ithaca, NY (Remote) 
•	Conducted over 60 customer discovery interviews for market validation research
•	technical expertise in polymer chemistry 
•	NSF I-Corps program
•	Agrochemical product development
Summer 2025 - AI Trainer, Materials Science Expert, Handshake MOVE (Remote)
•	Designed test questions for large language models to improve performance
•	Materials Science domain
Winter 2024-Spring 2025 - Post Doctoral Lab Support in Yang Lab, Cornell University, Ithaca, NY
•	Built a custom Initiated Chemical Vapor Deposition (iCVD) Reactor
•	Designed a vacuum chamber using Autodesk software which is now in use
•	Built software and automated the reactor using open-source Python libraries
•	Trained new users on operation and maintenance of reactor equipment and software
•	Soldered pin-outs for custom serial cables
Fall 2019-Winter 2024 - PhD Student in Yang Lab, Cornell University, Ithaca, NY
•	Wrote and published four peer-reviewed articles
•	Developed and validated a first-principles theory of iCVD kinetics (published)
•	Developed and validated a first-principles model of heat transfer in vacuum for the iCVD system (published)
•	Developed and validated a mixing model for the iCVD system and validated it against data (published)
•	Developed procedures to prevent defect formation in polymer thin film synthesis (published)
•	Measured mechanism and kinetics of perfluorinated film decomposition important for chemical etching and PFAS exposure risk (published)
•	Acted as Lab Safety officer, addressing and taking actions to prevent minor safety incidents or red flags and performing department level walkthroughs
•	Spot welded thermocouples for precision temperature measurements of sensitive vacuum components
•	Developed a new low cost, modular and precise method of composition sensing for rare gases

Spring 2016- Fall 2018 - Researcher, Aston Research Group, University of Idaho, Moscow, ID
•	Lead self-guided surface science and polymer research 
•	wet chemistry
•	spectral analysis and manipulation
•	principal components analysis for reaction product characterization 
•	Demonstrate root causes of a hydrophobic surface transition to be a low temperature degradation reaction

Summer 2018 - Engineering Intern, Glanbia Nutritionals, Corona, Ca
•	Recommended procedures to decrease startup time based on residence time distribution models
•	Discovered and provided recommendations to mitigate negative impacts of improperly installed equipment
•	Recommended optimization of cooling water flow rate for positive displacement, centrifugal and vacuum pumps totaling around 1000 gal per day of possible water savings
•	Wrote a preventative maintenance schedule for a piece of equipment and trained operators
•	Identified key water losses through process analysis and provided mitigation recommendations based on cooling water optimization, misattribution of water losses and incorrect installation
•	Assessed yield losses for a high profit margin customer using historical data
•	Compiled data to trend bottle filling malfunctions over time
•	Acted as project lead in the project engineer’s absence, coordinating with contractors to ensure work was safe and in accordance with agreed upon deliverables
•	Provided suggestions to increase fill efficiency during a bottle redesign based on observed incompatibility with process equipment
•	Discovered incorrect installation of a vacuum pump indicating available savings of up to 15000 gallons per day while operating
•	Determined meaning of flow totalizer readings and corrected a misattribution of water losses
•	Provided recommendations for proper chemical storage
Summer 2016 - Biological Science Aide, USDA-ARS, Adams, OR
•	Analyzed near infrared (NIR) spectra in De Light and Excel using chemometric techniques to calibrate an on-combine protein sensor for wheat
•	Collected NIR spectra of wheat samples to determine protein content
•	Collected gas samples for determining greenhouse gas emissions from soil and plant matter
•	Conducted soil moisture analysis on soil samples
Fall 2015 - Spring 2016 - Disability Support Services Notetaker: University of Idaho, Moscow, ID
•	Took lecture notes for students with disabilities under the Americans with Disabilities Act
  
Skills
======

Software: Microsoft Office products, Matlab, Python, Eclipse individual development environment, ImageJ, and Mathcad, Autodesk products (CFD and inventor), C++, SolidWorks, Aspen Plus,

Laboratory: initiated chemical vapor deposition (iCVD), quartz crystal microbalance (QCM-D), infrared spectroscopy (FTIR and NIR), scanning electron microscopy (SEM), scanning electron microscopy with energy dispersive x-ray spectroscopy (SEM/EDS), variable angle spectroscopic ellipsometry (VASE), dynamic contact angle (DCA), electrospinning, x-ray reflectivity (XRR), spin coating, atomic force microscopy (AFM),X-ray photoelectron spectroscopy (XPS)

Other: Scientific writing, experimental design, principal components analysis (PCA), data analysis and visualization, soldering, spot welding, vacuum equipment 


Publications
======
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>
  
Service and leadership
======
President, Scientific Vitality Coalition, Montreal, QC (Remote) 		    Spring 2025-present
•	Executive function for a small team
•	Manage weekly meetings and initiatives 
•	Volunteer capacity
University of Idaho Humanitarian Engineering Corps: 		           Fall 2015-Summer 2019
•	Acted as secretary by taking meeting minutes and coordinating club events and fundraisers
•	Acted as fundraising chair, taking responsibility for raising sufficient funds to travel
•	Implemented water system development in Carani, Bolivia 
•	Coordinated efforts to increase local service
Cornell Contra Dance Graduate Student Association:				Fall 2020- Spring 2020
•	Act as treasurer and help coordinate dances
Chemical Engineering Graduate Student Association: (Cornell)		Fall 2020- Winter 2025
•	Served as a representative for PhD students during all years of PhD
•	Organized events over the COVID-19 pandemic to facilitate community in the department
•	Served as vice president of the organization during the 2022/2023 academic year
•	Served as president during the 2023/2024 academic year
Local Service:									     Intermittent
•	Lead and contributed to multiple service days in Moscow and the surrounding area including Habitat for humanity construction, grounds keeping, labor and publicity for local businesses and non-profits

